export class MinimaxAI {
    ...
    getMove(board, winStrategy) {
        ...
        let bestMove = emptyCells[0]; // fallback
    ...
    minimax(board, winStrategy, depth, maxDepth, isMaximizing, aiPlayer, startTime) {
        // Time budget check
        if (Date.now() - startTime > this.timeBudget) {
    ...
    evaluateBoard(board, winStrategy, aiPlayer) {
        const humanPlayer = aiPlayer === 'X' ? 'O' : 'X';
        let score = 0;
    ...
    countThreats(board, player, winStrategy) {
        let threats = 0;
        const size = board.size;
        const winLength = winStrategy.winLength || 3;
        // Check rows
            ...
            for (let col = 0; col <= size - winLength; col++) {
                ...
                for (let i = 0; i < winLength; i++) {
                    if (board.cells[row][col + i] === player)
                        count++;
                    else if (board.cells[row][col + i] === null)
                        empty++;
                }
                if (count > 0 && count + empty >= winLength)
                    threats += count;
            }
        }
        // Check columns
            ...
            for (let row = 0; row <= size - winLength; row++) {
                ...
                for (let i = 0; i < winLength; i++) {
                    if (board.cells[row + i][col] === player)
                        count++;
                    else if (board.cells[row + i][col] === null)
                        empty++;
                }
                if (count > 0 && count + empty >= winLength)
                    threats += count;
            ...
            for (let col = 0; col <= size - winLength; col++) {
                ...
                for (let i = 0; i < winLength; i++) {
                    if (board.cells[row + i][col + i] === player)
                        count++;
                    else if (board.cells[row + i][col + i] === null)
                        empty++;
                }
                if (count > 0 && count + empty >= winLength)
                    threats += count;
